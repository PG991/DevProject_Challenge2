...
class AudioResNet18(nn.Module):
    """From-scratch ResNet-18 specialized for 1-channel audio spectrograms"""

    def __init__(self, n_classes, drop_p=0.2):
        super().__init__()
        # GPU-based Mel + dB pipeline
        self.mel_transform = T.MelSpectrogram(sample_rate=44100, n_fft=2048,
                                             hop_length=512, n_mels=128, power=2.0)
        self.db_transform  = T.AmplitudeToDB()
        self.spec_augment  = SpecAugment(time_mask_param=30, freq_mask_param=15, num_masks=2)
