...
class ResidualBlock(nn.Module):
    def __init__(self, in_c, out_c, stride=1, drop_p=0.3):
        super().__init__()
        self.conv1 = nn.Conv2d(in_c, out_c, 3, stride, 1)
        self.bn1   = nn.BatchNorm2d(out_c)
...
class AudioResNet(nn.Module):
    def __init__(self, n_mels, n_steps, n_classes, drop_p=0.3):
        ...
        self.layer1 = self._make_layer(32,  32, 2, stride=1, drop_p=drop_p)
        self.layer2 = self._make_layer(32,  64, 2, stride=2, drop_p=drop_p)
        self.layer3 = self._make_layer(64, 128, 2, stride=2, drop_p=drop_p)
        self.layer4 = self._make_layer(128,256, 2, stride=2, drop_p=drop_p)

        self.avgpool = nn.AdaptiveAvgPool2d((1,1))
        self.dropout = nn.Dropout(drop_p)
        self.fc      = nn.Linear(256, n_classes)

    def _make_layer(self, in_c, out_c, blocks, stride, drop_p):
        layers = [ResidualBlock(in_c, out_c, stride, drop_p)]
        for _ in range(1, blocks):
            layers.append(ResidualBlock(out_c, out_c, drop_p=drop_p))
        return nn.Sequential(*layers)
