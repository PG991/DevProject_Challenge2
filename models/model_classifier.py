...
class AudioResNet18(nn.Module):
    ...
    def __init__(self, n_classes, drop_p=0.15):
        ...
        # ResNet-18: 2x BasicBlock pro Stage
    ...
    def forward(self, wave):
        """
        wave: Tensor of shape (B,1,Time) auf GPU
        """
        # 1) Mel-Spektrogramm auf GPU erzeugen
        spec = self.mel_transform(wave)      # -> (B,1,n_mels,Time')
        spec_db = self.db_transform(spec)    # -> (B,1,n_mels,Time')

        # 2) Direkt verwenden (4D-Tensor für Conv2d)
        x = spec_db                         # -> (B,1,n_mels,Time')
